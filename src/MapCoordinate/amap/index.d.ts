declare global {
	namespace AMap {
		...
		class Map {
			...
			clearMap(): void;
			destroy(): void;
			plugin(name: string | Array<any>, callback: () => void): void;
			...
			getPitch(): number;
			on(eventName: string, handler: (e: MapsEvent) => void, context?): void;
		}
		...
		class Driving {
			...
			search(
				data: DirvingSearchByKeywords[],
				cb: (status: string, results: { info: string; origin: LngLat; destination: LngLat }) => void
			): void;
			search(
				pointA: AMap.LngLat,
				pointB: AMap.LngLat,
		...
		interface DirvingSearchByKeywords {
			keyword: string;
			city?: string;
		}
		class Geocoder {
